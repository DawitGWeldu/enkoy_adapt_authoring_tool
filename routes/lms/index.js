...
if (process.env.ENABLE_LMS_OPEN_TOKEN === 'true' || true) {
  ...
  try {
    ...
    if (!currentUser || !currentUser._id) {
      ...
      var errorHtml = [
        ...
        '<script>',
        'if (window.opener) {',
        '  window.opener.postMessage({ type: "ADAPT_TOKEN", success: false, error: "Not logged in" }, "' + lmsOrigin + '");',
        '}',
    ...
    var successHtml = [
      ...
      '<script>',
        'if (window.opener) {',
        '  window.opener.postMessage({ type: "ADAPT_TOKEN", success: true, token: "' + issued.token + '", userId: "' + issued.userId + '", tenantId: "' + issued.tenantId + '" }, "' + lmsOrigin + '");',
        '}',
